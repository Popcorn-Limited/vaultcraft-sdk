...
import { Stader } from "../../../../src/yieldOptions/providers/protocols/index.js";
import { publicClient } from "../../../setup.js";

const stader = new Stader();
...
describe.concurrent("read-only", () => {
    ...
    test("getAssets() should return all the available assets", async () => {
        const result = await stader.getAssets(1);
        expect(result.length).toBeGreaterThan(0);
    });

    test("getApy() should return the APY for a given asset", async () => {
        const result = await stader.getApy(1, "0xBCe0Cf87F513102F22232436CCa2ca49e815C3aC");
