import { Address, Chain, PublicClient, ReadContractParameters, createPublicClient, getAddress, http } from "viem"
...
export async function getVaults({ vaults, account = ADDRESS_ZERO, client }: { vaults: Address[], account?: Address, client: PublicClient }): Promise<VaultData[]> {
  ...
  metadata = metadata.map((entry, i) => {
    const key = `${networkMap[client.chain.id].name.toLowerCase()}:${entry.asset.address}`
